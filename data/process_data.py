...
def process_data():
    print('processing directories of pdbstyle 2.07')
    a = 0
    error = []
    data = []
    START = 0
        ...
        for root, dirs, files in os.walk('./pdbstyle-2.07', topdown=True):
            ...
            for f in files:
                if f[-4:] == '.ent':
                    ...
                    pdb,CLS,SRC,region = util.parse_pdb(root+'/'+f)

                    if CLS[0] in 'hijkl': continue # skip these classes
                    ...
                    data.append( {'nID':nID, 'sID':sID, 'CA':CA, 'SEQ':SEQ, 'CLS':CLS, 'SRC':SRC, 'REGION':region} )
                    a+=1
    except:
        #print(pdb.keys())
        #print(s)
    ...
    util.file_list(IGNORE, 'scope_data.skipped')
